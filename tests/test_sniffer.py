...
OPEN_ACCESS_RULES = os.path.join(
    os.path.dirname(__file__),
    "..",
    "test_targets",
    "open_access.rules",
)
AUTH_NO_OWNER_RULES = os.path.join(
    os.path.dirname(__file__),
    "..",
    "test_targets",
    "auth_no_owner.rules",
)
WEAK_UID_RULES = os.path.join(
    os.path.dirname(__file__),
    "..",
    "test_targets",
    "weak_uid.rules",
...
def test_clean_file_has_no_findings(tmp_path):
    ...
    clean_path = tmp_path / "temp_clean.py"
    clean_path.write_text(
        "def hello():\n    print('Hello, world!')\n",
        encoding="utf-8",
    )

    findings = scan_file(str(clean_path))
